...
import { useState } from "react"; // useState를 사용할 수 있게됨. setSearch, setRegion, setIsModalOpen 등의 함수를 사용할 수 있게됨.
...
const HomeSectionFilterWrapper = ({ title, category, allPosts, basePath }: Props) => {
  const [search, setSearch] = useState(""); // const [search, setSearch]: 값과 상태를 업데이트하는 함수, useState: 상태 관리 훅, 초기값은 빈 문자열
  const [region, setRegion] = useState("전체 지역"); // 전체지역이라는 문자열을 초기값으로 설정
